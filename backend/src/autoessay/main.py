...
def _count_active_essays(session: Session, user_id: str) -> int:
    ...
    - the project has no run yet AND was created within the last 24h
      (grace window so the New Run page still counts before submit),
    - or the latest non-deleted run is not in
      :data:`LIMIT_TERMINAL_STATES`.

    Projects created without ever kicking off a run used to consume a
    slot forever, even though the UI does not surface them once
    abandoned. That made the limit feel inflated. Such projects now
    fall out of the count after 24h.
    """
